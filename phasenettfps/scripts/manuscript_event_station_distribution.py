...
Requrements from Shawn:
Map of our study region with only manual PS arrivals and a cartoon. Stations are color-coded with the number of PS arrivals.
"""
...
def plot_stations(fig: pygmt.Figure):
    ...
    df_stations_catalog = pd.read_csv(
    ...
    fig.colorbar(
        position="JMB+o0c/3.2c+w5i/0.3i",
        frame=["x+lNumber of manually picked PS arrivals"],
    )
...
def plot_events(fig: pygmt.Figure):
    ...
    fig.colorbar(
        ...
        frame=["x+lManually picked earthquake depth (km)"],
    )
